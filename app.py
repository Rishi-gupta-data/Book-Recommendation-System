...
model_path = project_root / "models" / "latest_model.pkl"
recommender = BookRecommender(model_path)

# Try loading pickle model first
if not recommender.load_model():
    logger.warning(f"Failed to load pickle model from {model_path}")
    
    # Fallback: Try loading from portable format
    portable_dir = project_root / "models" / "portable"
    if portable_dir.exists():
        logger.info("Attempting to load model from portable format...")
        if recommender.load_from_portable(portable_dir):
            logger.info("✅ Portable model successfully loaded")
        else:
            error_msg = "Failed to load both pickle and portable models"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
    else:
        error_msg = (
            f"Neither pickle model ({model_path}) nor "
            f"portable model directory ({portable_dir}) found"
        )
        logger.error(error_msg)
...
if __name__ == "__main__":
    app.run(debug=True, port=5000)  # Added explicit port for clarity